...
class window.AutoComplete2
  ...
  constructor: (options={}) -> 
    ...
    @template_for_list_item =  if options.template_for_list_item? then options.template_for_list_item else  "<div data-auto_complete_id='list_item_id' 
                                                                                                                  data-auto_complete_str='escaped_str' 
                                                                                                                  class='list_item' onclick='list_item_pick_onclick_text'>
                                                                                                               <span class='text'>list_item_str</span>
                                                                                                               <span class='icon'>+</span>
                                                                                                             </div>"
    
    @template_for_picked_item = if options.template_for_picked_item? then options.template_for_picked_item else   "<div class='picked_item flat_button grey' 
                                                                                                                      data-auto_complete_id=''
                                                                                                                      onclick='AutoComplete2.INSTANCE.acp.delete(this)'>
                                                                                                                        <span class='text'></span>
                                                                                                                        <span class='icon'>x</span>
                                                                                                                    </div>"   
    @acd = new AutoCompleteDisplay2( page_id: @page_id )
    @acp = new AutoCompletePick2( page_id: @page_id, fill_input_with_pick: @fill_input_with_pick, picked_items_box: picked_items_box, template_for_picked_item: @template_for_picked_item, on_delete: @acd.on_delete, on_pick: @acd.on_pick )
    @acs = new AutoCompleteSearch2( full_list: @full_list, search_input_field: search_input_field, search_results_box: search_results_box, pick_onclick_text: "AutoComplete2.INSTANCE.acp.pick(this)", on_auto_complete: @acd.on_auto_complete, template_for_list_item: @template_for_list_item, dont_use_first_letter_list: @dont_use_first_letter_list, show_all_on_first_letter: @show_all_on_first_letter )
...
class window.AutoCompleteSearch2
  ...
  constructor: (options={}) ->   
    ...
    unless options.search_input_field?
     $.error "AutoCompleteSearch2 requires 'options.search_input_field' for which is monitored for text input. E.g. ac = new AutoCompleteSearch2(search_input_field: $('#invite .auto_complete_exp' ))" 
    @search_input_field = options.search_input_field
    
    unless options.search_results_box?
      $.error "AutoCompleteSearch2 requires 'options.search_results_box' output of the results list. E.g. ac = new AutoCompleteSearch2(search_results_box: $('#invite .auto_complete_search_results_box' ))" 
    @search_results_box = options.search_results_box
    
    unless options.pick_onclick_text?
      $.error "AutoCompleteSearch2 requires 'options.pick_onclick_text'." 
    @pick_onclick_text = options.pick_onclick_text
    
    unless options.template_for_list_item?
      $.error "AutoCompleteSearch2 requires 'template_for_list_item"
    ...
    unless options.full_list? 
      $.error "AutoCompleteSearch2 requires 'options.full_list'"
    @full_list = options.full_list 
  ...
  setup_event_listner_on_search_input: =>    
    @search_input_field.on "keyup", @handle_search_input_key_up
  
  handle_search_input_key_up: (event) =>  
...
class window.AutoCompletePick2
  ...
  pick: (element) => 
    ...
    added_element.find(".text").html(picked_str)
...
class AutoCompleteDisplay2
  ...
  add_event_handlers: =>
    # remove it first in case we had added it in a previous instantiation.
    $("##{@page_id} .auto_complete_search_results_box").off("touchstart", @list_touched)
    $("##{@page_id} .auto_complete_search_results_box").on("touchstart", @list_touched)
    
    $("##{@page_id} .auto_complete_search_input_field").off("focus", @search_input_focused)
    $("##{@page_id} .auto_complete_search_input_field").on("focus", @search_input_focused)
    $("##{@page_id} .auto_complete_search_input_field").off("keyup", @search_input_focused)
    $("##{@page_id} .auto_complete_search_input_field").on("keyup", @search_input_focused)
    
    $("##{@page_id} .cancel_button").off("touchstart", @cancel_done)
    $("##{@page_id} .cancel_button").on("touchstart", @cancel_done)
    
    $("##{@page_id} .done_button").off("touchstart", @cancel_done)
    $("##{@page_id} .done_button").on("touchstart", @cancel_done)
    
  list_touched: => @hide_keyboard()
  
  cancel_done: => 
    @hide_keyboard()
  
  hide_keyboard: => Keyboard.hide $("##{@page_id} .auto_complete_search_input_field") 
  ...
  on_pick: =>
    ...
    $("##{@page_id} .auto_complete_search_input_field").attr("placeholder", "Type another")
